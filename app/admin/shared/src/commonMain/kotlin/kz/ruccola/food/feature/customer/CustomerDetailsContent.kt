...
@Composable
fun CustomerDetailsContent(customer: CustomerDetailsDto, modifier: Modifier = Modifier) {
    Column(
        modifier = modifier.padding(16.dp).verticalScroll(rememberScrollState()),
        verticalArrangement = Arrangement.spacedBy(16.dp),
    ) {
        SectionCard(icon = Icons.Outlined.ManageAccounts, title = "Personal Info") {
            DetailRow(label = stringResource(Res.string.first_name), value = customer.firstName)
            HorizontalDivider()
            DetailRow(label = stringResource(Res.string.last_name), value = customer.lastName)
            HorizontalDivider()
            DetailRow(label = stringResource(Res.string.email), value = customer.email)
            HorizontalDivider()
            DetailRow(
        ...
        if (plan != null) {
            SectionCard(icon = Icons.Filled.CalendarMonth, title = stringResource(Res.string.subscription)) {
                DetailRow(label = stringResource(Res.string.calories), value = plan.calories.toString())
                HorizontalDivider()
                DetailRow(
                ...
                HorizontalDivider()
                DetailRow(label = stringResource(Res.string.price_per_day_short), value = plan.pricePerDay.toString())
                HorizontalDivider()
                DetailRow(label = stringResource(Res.string.total), value = (plan.pricePerDay * plan.days).toString())
                HorizontalDivider()
                DetailRow(label = stringResource(Res.string.start_date), value = plan.chosenDate.toString())
            }
            SectionCard(icon = Icons.Outlined.Settings, title = stringResource(Res.string.preferences)) {
                PrefRow(label = stringResource(Res.string.needs_cutlery), value = customer.prefs.needsCutlery)
                HorizontalDivider()
                PrefRow(label = stringResource(Res.string.weekend_delivery), value = customer.prefs.weekendDelivery)
                HorizontalDivider()
                PrefRow(label = stringResource(Res.string.morning_delivery), value = customer.prefs.morningDelivery)
            }
        }
    }
}
...
@Composable
internal fun SectionCard(icon: ImageVector, title: String, content: @Composable () -> Unit) {
...
@Composable
private fun PrefRow(label: String, value: Boolean) {
    Row(
        modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp),
        horizontalArrangement = Arrangement.SpaceBetween,
        verticalAlignment = Alignment.CenterVertically,
