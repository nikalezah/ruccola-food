...
suspend fun <T> Query.toPagingResponse(page: Int, size: Int, transform: suspend (ResultRow) -> T): PagingResponse<T> {
    val totalCount = this.count()
    val items = this.copy().limit(size).offset((page * size).toLong()).toList().map { transform(it) }

    return PagingResponse(items = items, totalCount = totalCount, page = page, size = size)
}
