...
class MealPlanDayService {
    ...
    suspend fun getCurrent(): MealPlanDayDto? = dbQuery {
        MealPlanDays.selectAll().where { MealPlanDays.current eq true }.singleOrNull()?.let { toDto(it) }
    }

    suspend fun advanceCurrentToNext() = dbQuery {
        val serialOfCurrent =
            MealPlanDays.select(MealPlanDays.serial)
    ...
    suspend fun getAll(language: Language): List<MealPlanDayDto> = dbQuery {
        val allDays = MealPlanDays.selectAll().orderBy(MealPlanDays.serial to SortOrder.ASC).toList()
    ...
    suspend fun save(id: Int?, dishIdToMeal: Map<Int, Meal>, language: Language): Result<MealPlanDayDto> = dbQuery {
        val mpdDto =
            if (id == null) {
                val max =
                    MealPlanDays.select(MealPlanDays.serial.max()).singleOrNull()?.get(MealPlanDays.serial.max()) ?: 0
                ...
                val mdp =
                    MealPlanDays.selectAll().where { MealPlanDays.id eq id }.singleOrNull()?.let(::toDto)
        ...
        val dishes =
            MealPlanDayDishes.batchInsert(dishIds) { dishId ->
        ...
        val dishDtoList =
            Dishes.selectAll().where { Dishes.id inList dishIds }.toList().map { dishService.toDto(it, language) }

        val dishWithMeal = dishes.map { (dishId, meal) ->
            DishWithMealDto(dishDtoList.first { it.id == dishId }, Meal.valueOf(meal))
        }

        Result.success(MealPlanDayDto(mpdDto.id, mpdDto.serial, mpdDto.current, dishWithMeal))
    }

    suspend fun delete(id: Int): Int = dbQuery { MealPlanDays.deleteWhere { MealPlanDays.id eq id } }

    suspend fun getDishes(mealPlanDayId: Int, language: Language): Result<List<DishWithMealDto>> = dbQuery {
        MealPlanDays.selectAll().where { MealPlanDays.id eq mealPlanDayId }.count() > 0 ||
                return@dbQuery Result.failure(NoSuchElementException("MealPlanDay not found"))

        val rows = MealPlanDayDishes.selectAll().where { MealPlanDayDishes.mealPlanDayId eq mealPlanDayId }.toList()
        ...
        val dishMap =
            if (dishIds.isEmpty()) {
                ...
                Dishes.selectAll()
                    .where { Dishes.id inList dishIds }
                    .toList()
    ...
    suspend fun setCurrent(id: Int): Result<MealPlanDayDto> = dbQuery {
    ...
    suspend fun reorder(ids: List<Int>): Result<Boolean> = dbQuery {
    ...
    fun toDto(row: ResultRow): MealPlanDayDto =
        MealPlanDayDto(row[MealPlanDays.id].value, row[MealPlanDays.serial], row[MealPlanDays.current])
}
